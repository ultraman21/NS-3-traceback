...
Ipv4L3Protocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Ipv4L3Protocol")
    ...
    .AddAttribute ("DefaultIdentification",
                   "The TTL value set by default on "
                   "all outgoing packets generated on this node.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&Ipv4L3Protocol::m_Id),
...
////////////////////////////////////
////////////////////////////////////
  if ( device->GetIfIndex() == 1 )
  {
    m_Id = 0x0000;
  }
  else
  {
    //std::cout<< "GetNDevices:" << m_node->GetNDevices () << "   device->GetIfIndex():" << device->GetIfIndex()<< std::endl;
    m_Id = ipHeader.GetIdentification() * ((m_node->GetNDevices () - 2) + 1) + (device->GetIfIndex()-1) + 1 ;
    //std::cout << "Netdevice "<< m_node->GetId() << "'s Interfaces: " << m_node->GetNDevices () << "  packet UID:" << packet->GetUid() << std::endl;

  }
  //std::cout << "Node "<< m_node->GetId() << " receive from Interface: " << device->GetIfIndex() << "  packet UID:" << packet->GetUid() << std::endl;
  ...
  if (route && route->GetGateway () != Ipv4Address ())
    {
      ...
      ipHeader = BuildHeader_Id (source, destination, protocol, packet->GetSize (), ttl, tos, m_Id);
      //std::cout<<"m_Id = "<<m_Id<<std::endl;
...
{
  NS_LOG_FUNCTION (this << source << destination << (uint16_t)protocol << payloadSize << (uint16_t)ttl << (uint16_t)tos << s_Identification);
  Ipv4Header ipHeader;
  ipHeader.SetSource (source);
  ipHeader.SetDestination (destination);
  ipHeader.SetProtocol (protocol);
  ...
  ipHeader.SetIdentification (s_Identification);
...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  ipHeader.SetIdentification(m_Id);
